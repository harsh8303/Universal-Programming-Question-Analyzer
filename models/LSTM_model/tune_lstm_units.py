...
LEARNING_RATE = 5e-4
DROPOUT_RATE = 0.3
EMBEDDING_DIM = 128  # Locked from previous tuning
# ==========================================
...
def build_model(vocab_size, max_seq_length, num_classes, lstm1, lstm2):
    ...
    #  THE FIX: Explicit Dropout layers bypass the Keras 3 LSTM Masking bug
    x = Dropout(DROPOUT_RATE)(x)
    x = Bidirectional(LSTM(units=lstm1, return_sequences=True))(x)
    
    x = Dropout(DROPOUT_RATE)(x)
    x = Bidirectional(LSTM(units=lstm2, return_sequences=True))(x)
...
def run_lstm_experiments():
    ...
    for lstm1, lstm2 in LSTM_CONFIGS_TO_TEST:
        ...
        # RAM and Reproducibility safety restored!
