# models/LSTM_model/train_manual_tuned_lstm.py
import os
import sys
import json
import numpy as np
...
# ==========================================
# 2. LOCAL MAC WINNING HYPERPARAMETERS
# ==========================================
...
def build_model(vocab_size, max_seq_length, num_classes):
    ...
    model = Model(inputs=inputs, outputs=outputs, name="Universal_Problem_Analyzer_Final_BiLSTM")
...
def plot_training_history(history, save_dir):
    ...
    plot_path = os.path.join(save_dir, "final_bilstm_training_history.png")
    plt.tight_layout()
    plt.savefig(plot_path, dpi=300)
    print(f" Training plot saved at: {plot_path}")

def train_final_lstm():
    print("\n STARTING FINAL DEEP BiLSTM TRAINING (Locally Tuned Hyperparameters)...")
    ...
    with open(os.path.join(save_dir, "final_bilstm_model_summary.txt"), "w") as f:
        model.summary(print_fn=lambda x: f.write(x + '\n'))
    
    best_model_path = os.path.join(save_dir, "best_manual_tuned_bilstm.keras")
    ...
    with open(os.path.join(save_dir, "final_bilstm_training_history.json"), "w") as f:
        json.dump(history.history, f)
        
    plot_training_history(history, save_dir)
    
    print("\n Evaluating Final Deep BiLSTM Model on Unseen Test Set...")
