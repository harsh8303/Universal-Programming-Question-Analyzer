# models/GRU_model/train_kerastuner_gru.py
import os
import sys
import json
import numpy as np
...
def build_model(vocab_size, max_seq_length, num_classes):
    ...
    x = Bidirectional(GRU(units=BEST_GRU_1, return_sequences=True), name="BiGRU_Layer_1")(x)
    
    x = Dropout(BEST_DROPOUT, name="Dropout_2")(x)
    x = Bidirectional(GRU(units=BEST_GRU_2, return_sequences=True), name="BiGRU_Layer_2")(x)
    ...
    model = Model(inputs=inputs, outputs=outputs, name="Universal_Problem_Analyzer_Final_BiGRU")
...
def plot_training_history(history, save_dir):
    ...
    plt.title('Final BiGRU Model Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()
    ...
    plt.title('Final BiGRU Model Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
    
    plot_path = os.path.join(save_dir, "final_bigru_training_history.png")
    plt.tight_layout()
    plt.savefig(plot_path, dpi=300)
    print(f" Training plot saved at: {plot_path}")

def train_final_gru():
    print("\n STARTING FINAL DEEP BiGRU TRAINING (Colab Hyperparameters)...")
    ...
    with open(os.path.join(save_dir, "final_bigru_model_summary.txt"), "w") as f:
        model.summary(print_fn=lambda x: f.write(x + '\n'))
    
    best_model_path = os.path.join(save_dir, "best_kerastuner_bigru.keras")
    
    # Syncing all callbacks to val_loss
    ...
    with open(os.path.join(save_dir, "final_bigru_training_history.json"), "w") as f:
        json.dump(history.history, f)
        
    plot_training_history(history, save_dir)
    
    print("\n Evaluating Final Deep Bi-GRU Model on Unseen Test Set...")
