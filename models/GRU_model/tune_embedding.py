# models/GRU_model/tune_embedding.py
import os
import sys
import json
import numpy as np
...
# ==========================================
# CHANGE: Testing different Embedding Dimensions
...
def build_model(vocab_size, max_seq_length, num_classes, emb_dim):
...
def run_embedding_experiments():
    print("🚀 STARTING AUTOMATED EMBEDDING DIMENSION TUNING LOOP...")
    ...
    for emb_dim in EMBEDDING_DIMS_TO_TEST:
        ...
        model = build_model(vocab_size, max_seq_length, num_classes, emb_dim)
        ...
        experiment_results[str(emb_dim)] = {
    ...
    # ==========================================
    json_path = os.path.join(CURRENT_DIR, "embedding_tuning_results.json")
    ...
    # ==========================================
    print("\n\n" + "*"*25)
    print("      FINAL EMBEDDING TUNING REPORT")
    print("*"*25)
    print(f"{'Emb Dim':<10} | {'Best Epoch':<12} | {'Best Val Acc':<15} | {'Best Val Loss':<15}")
    print("-" * 60)
    for dim_val, metrics in experiment_results.items():
        print(f"{dim_val:<10} | {metrics['Best_Epoch']:<12} | {metrics['Best_Validation_Accuracy']:<15.4f} | {metrics['Best_Validation_Loss']:<15.4f}")
    print("-" * 60)
