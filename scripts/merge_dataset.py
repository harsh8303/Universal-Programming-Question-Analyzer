...
def merge_all_platforms():
    ...
    for file in raw_files:
        ...
        if "raw" in filename.lower() or filename == "gfg.csv":
            print(f" Skipping irrelevant file: {filename}")
            continue
    ...
    # 2. Clean up duplicates (Smart Anti-Overfitting Deduplication)
    print("  Removing exact description duplicates to prevent overfitting...")
    master_df['desc_lower'] = master_df['description'].astype(str).str.lower().str.strip()
    master_df.drop_duplicates(subset=['desc_lower'], keep='first', inplace=True)
    master_df.drop(columns=['desc_lower'], inplace=True) # temp column delete

    print("  Shuffling the dataset for unbiased model training...")
    ...
    output_path = os.path.join(config.DATA_DIR, "merged", "unified_programming_problems.csv")
